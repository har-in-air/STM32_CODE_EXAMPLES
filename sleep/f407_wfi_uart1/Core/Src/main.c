...
// Use WFI instruction to put the CPU to sleep. Wake up on PA0 button press
// to transmit a message from uart1, returns to main thread, again calls WFI
// to sleep.
// Note that any interrupt will wake up the CPU so we need to suspend the
// SysTick timer, prior to calling wfi.
...
int main(void){
    ...
    // Stop Systick Timer else the tick interrupt will wake up the cpu
    HAL_SuspendTick();

    uint32_t counter = 0;
    while (1) {
    	__WFI(); // uses inline assembly to call wfi thumb instruction

    	// if WFI is commented out, you will see the board led toggling every
    	// second approximately
    	// if WFI is called, the while loop is suspended until PA0 button
    	// is pressed. Then the interrupt callback transmits from UART1 and returns
    	// to the while loop, so you will see the LED toggle once only before
    	// wfi is called again.

    	// CPU resumes here on return from interrupt
    	HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_1);
    	// can't use HAL_Delay() here as ticks are suspended
    	while (counter < 0x8FFFFF) {
    		counter++;
    		}
    	counter = 0;
        }
...
static void GPIO_Init(void){
    ...
    // Configure GPIO pin : PA0  board built in button as external interrupt
...
char msgToSend[] = "Hello from EXTI0 pin interrupt handler\r\n";
