...
// Use ARM Cortex M4 sleep on exit feature to periodically transmit every 100mS from uart1  using timer6
// with interrupt.
// Cpu goes to sleep on exit from TIM6 periodic interrupt, stays in interrupt mode (no unstacking), and
// wakes up on next TIM6 interrupt after 100mS
// So all peripheral initialization should be done prior to enabling sleep-on-exit feature.
...
char uartXmitMsg[] = "Some random message every 100mS\r\n";
...
int main(void){
    ...
    // make sure all initialization is complete before enabling sleep on exit feature
    TIM6_Init();

    // sleep on exit is an ARM Cortex M4 feature, System Control Block peripheral, SCR register
    // if enabled, cpu will go to sleep on exit of any interrupt, and will wake up from sleep
    // on an interrupt of any priority (different from what ARM specifies,
    // which is that only interrupts of higher priority can wake up the cpu)
#if 1
    HAL_PWR_EnableSleepOnExit();
#endif

    // clear the TIM6 interrupt status register to avoid spurious interrupts before we're ready
    TIM6->SR = 0;
    // start the basic timer periodic counter with interrupt callback on period elapse
    HAL_TIM_Base_Start_IT(&htim6);

    while (1) {
    	// if Sleep on exit call is enabled, then while loop should execute only once
    	// (before the first TIM6 period elapse interrupt)
    	// toggle board LED every second
...
static void TIM6_Init(void){
    // TIM6 is on APB1 (see stm32f407xx.h_
	...
	htim6.Instance = TIM6;
    htim6.Init.Prescaler = 499; // divide by 500, i.e. counter clk = 100000
	htim6.Init.CounterMode = TIM_COUNTERMODE_UP;
	htim6.Init.Period = 9999; // want cntrclock / period  = 10Hz for a 100 ms interval
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef * htim) {
	if ( HAL_OK !=  HAL_UART_Transmit(&huart1, (uint8_t*) uartXmitMsg, (uint16_t)strlen(uartXmitMsg), HAL_MAX_DELAY) ) {
		Error_Handler();
		}
	}
