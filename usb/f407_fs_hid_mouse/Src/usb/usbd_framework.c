...
extern void hid_write_mouse_report();
...
static void usbf_process_request(){
	...
	switch(pRequest->bmRequestType & (USB_BM_REQUEST_TYPE_MASK | USB_BM_REQUEST_TYPE_RECIPIENT_MASK))	{
		...
		default :
		log_error("usbf_process_request: unknown request");
		break;		
		}
	}
...
static void usbf_process_class_interface_request(){
	...
	switch(pRequest->bRequest)	{
		case USB_HID_SETIDLE:
		    log_info("HID_SETIDLE request: Switching control transfer stage to IN-STATUS.");
			pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_STATUS_IN;
			break;
		default:
			log_error("Unknown class interface request");
			break;		
		}
	}
...
static void usbf_process_standard_interface_request(){
	...
	switch (pRequest->wValue >> 8)	{
		case USB_DESCRIPTOR_TYPE_HID_REPORT:
			...
			log_info("xxxx   Sizeof HID_Report_Descriptor = %d bytes", HID_Report_Desc_Size_Bytes);
		...
		default :
			log_error("unknown standard interface request");
			break;
		}
	}
