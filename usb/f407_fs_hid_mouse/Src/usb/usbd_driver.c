...
static void usbd_initialize_gpio_pins() {
	...
	// PB3 is used for SWO trace but it's dynamically configured by the debug application via the SWD adapter
	MODIFY_REG(GPIOA->AFR[1],
...
static void usbd_initialize_core() {
	// HNP, SRP, FS timeout calibration x don't care
	// set usb turnaround time set
	//-device initialization
	// nonzero length status OUT handshake x
	...
	// rm0090 pg 1279
	WRITE_REG(USB_OTG_FS->GINTSTS, 0xF73CFCFF);
...
static void usbd_rxflvl_handler() {
    ...
    case 0x03: // OUT data transfer (multiple packets) has completed
    	log_debug("out data transfer completed");
