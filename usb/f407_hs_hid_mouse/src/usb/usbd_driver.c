...
static void usbd_initialize_gpio_pins(){
	...
	// PB13 is VBUS sense (input)
	MODIFY_REG(GPIOB->MODER,
			GPIO_MODER_MODER13,
			_VAL2FLD(GPIO_MODER_MODER13, 0)
			);
	}
...
static void usbd_initialize_core(){
	...
	CLEAR_BIT(USB_OTG_HS->GCCFG, USB_OTG_GCCFG_NOVBUSSENS);
	// disable A device vbus sensing
	CLEAR_BIT(USB_OTG_HS->GCCFG, USB_OTG_GCCFG_VBUSASEN);
    ...
    //SET_BIT(USB_OTG_HS->GCCFG, USB_OTG_GCCFG_NOVBUSSENS);
    //CLEAR_BIT(USB_OTG_HS->GCCFG, USB_OTG_GCCFG_VBUSBSEN);
    //CLEAR_BIT(USB_OTG_HS->GCCFG, USB_OTG_GCCFG_VBUSASEN);
...
// Disconnects the USB device from the bus.
static void usbd_disconnect(){
...
static void usbd_usbrst_handler(){
	log_info("USB reset signal");
	// note that interrupt unmasking was done in core_initialize
	for (uint8_t i = 0; i <= ENDPOINT_COUNT; i++)	{
		usbd_deconfigure_endpoint(i);
		}
...
static void usbd_refresh_fifo_start_addresses(){
	// The first changeable start address begins after the region of RxFIFO which starts at 0
...
// Any change on any FIFO will update the the registers of all TxFIFOs to adapt the start offsets 
// in the FIFO dedicated memory.
static void usbd_configure_txfifo_size(uint8_t endpoint_number, uint16_t sizeBytes){
