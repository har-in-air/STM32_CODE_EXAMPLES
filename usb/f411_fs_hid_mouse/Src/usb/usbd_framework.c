...
static USB_DEVICE_t *pUSB_Device;
...
void usbd_initialize(USB_DEVICE_t *pDevice){
	pUSB_Device = pDevice;
...
static void usbf_reset_received_handler(){
	pUSB_Device->in_data_size = 0;
	pUSB_Device->out_data_size = 0;
	pUSB_Device->config_index = 0;
	pUSB_Device->device_state = USB_DEVICE_STATE_DEFAULT;
	pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_SETUP; // wait for setup token
...
static void usbf_setup_data_received_handler(uint8_t endpoint_number, uint16_t byte_count){
	...
	USB_Driver.read_packet(pUSB_Device->ptr_out_buffer, byte_count);
	//log_debug_array("SETUP data: ", pUSB_Device->ptr_out_buffer, byte_count);
...
static void usbf_process_request(){
	const USB_REQUEST_t  *pRequest = (const USB_REQUEST_t *)pUSB_Device->ptr_out_buffer;
	...
	switch(pRequest->bRequest)	{
		case USB_STANDARD_GET_DESCRIPTOR:
			...
			case USB_DESCRIPTOR_TYPE_DEVICE:
				...
				pUSB_Device->ptr_in_buffer = &Device_Descriptor;
				pUSB_Device->in_data_size = descriptor_length;

				log_info("ctrl xfer -> IN-DATA");
				pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_DATA_IN;
				break;
			...
			case USB_DESCRIPTOR_TYPE_CONFIGURATION:
				...
				pUSB_Device->ptr_in_buffer = &Config_Desc_Set;
				pUSB_Device->in_data_size = descriptor_length;
				log_info("ctrl xfer -> IN-DATA");
				pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_DATA_IN;
				break;
		...
		case USB_STANDARD_SET_ADDRESS:
			...
			log_info("std_set_addr received");
			...
			pUSB_Device->device_state = USB_DEVICE_STATE_ADDRESSED;
			log_info("ctrl xfer stage -> IN-STATUS.");
			pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_STATUS_IN;
			break;
		...
		case USB_STANDARD_SET_CONFIG:
			...
			log_info("Std_set_config received");
			pUSB_Device->config_index = pRequest->wValue;
		    usbd_configure();
			pUSB_Device->device_state = USB_DEVICE_STATE_CONFIGURED;
...
static void usbf_process_class_interface_request(){
	...
	switch(pRequest->bRequest)	{
		case USB_HID_SETIDLE:
		    log_info("HID_SETIDLE xfer stage-> IN-STATUS");
			pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_STATUS_IN;
			break;
		default:
			log_error("Unknown request");
...
static void usbf_process_standard_interface_request(){
	...
	switch (pRequest->wValue >> 8)	{
		case USB_DESCRIPTOR_TYPE_HID_REPORT:
			pUSB_Device->ptr_in_buffer = &HID_Report_Descriptor;
			pUSB_Device->in_data_size = HID_Report_Desc_Size_Bytes;
			//log_info("HID_Report_Descriptor %d bytes", HID_Report_Desc_Size_Bytes);

			log_info("xfer ctrl stage -> IN-STATUS.");
			pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_DATA_IN;
			break;
...
static void usbf_process_control_transfer_stage(){
	switch(pUSB_Device->control_transfer_stage)	{
		...
		case USB_CONTROL_STAGE_DATA_IN:
			...
			uint8_t data_size = MIN(pUSB_Device->in_data_size, Device_Descriptor.bMaxPacketSize0);

		    USB_Driver.write_packet(0, pUSB_Device->ptr_in_buffer, data_size);
		    pUSB_Device->in_data_size -= data_size;
		    pUSB_Device->ptr_in_buffer += data_size;
			...
			// implementation
			log_info("ctrl xfer -> IN-DATA IDLE");
		    pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_DATA_IN_IDLE;

			// finished multi-packet data transfer
		    if (pUSB_Device->in_data_size == 0)  {
		    	...
		    	if (data_size == Device_Descriptor.bMaxPacketSize0)	{
		    		log_info("ctrl xfer -> IN-DATA ZERO");
		    		pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_DATA_IN_ZERO;
		    		}
		    	else {
					// last data packet size was less than max packet size, so host
					// knows that transfer is complete, will acknowledge  status		    	
		    		log_info("ctrl xfer -> OUT-STATUS");
		    		pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_STATUS_OUT;
		    		}
		    	}
			break;
		case USB_CONTROL_STAGE_DATA_IN_IDLE:
			...
			// pUSB_Device->control_transfer_stage  to USB_CONTROL_STAGE_DATA_IN		
			break;
		case USB_CONTROL_STAGE_STATUS_OUT:
			...
			pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_SETUP;
			break;
...
static void usbf_in_transfer_completed_handler(uint8_t endpoint_number){
	...
	if (pUSB_Device->in_data_size)	{
		log_info("ctrl stage -> IN-DATA");
		pUSB_Device->control_transfer_stage = USB_CONTROL_STAGE_DATA_IN;
		}
	else 
	if (pUSB_Device->control_transfer_stage == USB_CONTROL_STAGE_DATA_IN_ZERO)	{
