...
// Use gen purpose timer TIM2 in PWM mode to fade in and out an LED with 1s intervals
...
int main(void){
    ...
    while (1) {
        while (brightness < htim2.Init.Period) {
            brightness += htim2.Init.Period/1000;  // take one second to brighten to max
            __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, brightness);
            // 1ms pwm period, so delay 1mS
            HAL_Delay(1);
            }
        while (brightness > 0) {
            brightness -= htim2.Init.Period/1000; // take one second to fade to dark
            __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, brightness);
            // 1ms pwm period, so delay 1ms
            HAL_Delay(1);
            }
...
void SystemClock_Config(void){
    ...
    // zero out the struct to ensure unused fields are safe (0)
...
static void TIM2_Init(void){
    ...
    htim2.Init.Prescaler = 4; // divide by 5, i.e. counter clk = 10,000,000
    htim2.Init.CounterMode = TIM_COUNTERMODE_UP;
    // TIM2 ARR is 32bit
    htim2.Init.Period = 10000-1; // 1 millisecond period
    // when pwm mode is used, need to use TIM_PWM_Init
    if (HAL_TIM_PWM_Init(&htim2) != HAL_OK)  {
        Error_Handler();
        }
    // now we need to configure the output channel
    // zero the struct and only set fields required for output channel pwm mode
    TIM_OC_InitTypeDef pwmConfig = {0};
    pwmConfig.OCMode = TIM_OCMODE_PWM1;
    // when counter < CCR1, we want the channel output to be high
    pwmConfig.OCPolarity = TIM_OCPOLARITY_HIGH;
    // start with 0% duty cycle, we will periodically change it to fade in and out the LED
    pwmConfig.Pulse = 0; 
