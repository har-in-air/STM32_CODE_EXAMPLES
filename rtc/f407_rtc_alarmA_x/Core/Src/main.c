...
// Demo RTC Alarm event
// Wakeup from standby mode @ 23:15:30 every day
...
int main(void){
    ...
    // on wake up from standby, system reset, check flags
...
void RTC_CalendarConfig(void){
	// set calendar to 23:15:00 PM, 14th june 2020 Sunday
...
void HAL_GPIO_EXTI_Callback(uint16_t gpioPin) {
	...
	printMsg("Set current time : %02d:%02d:%02d\r\n", rtcTime.Hours, rtcTime.Minutes, rtcTime.Seconds);
	printMsg("Set current date : 20%02d-%02d-%02d [%s]\r\n", rtcDate.Year, rtcDate.Month, rtcDate.Date, getWeekDay(rtcDate.WeekDay));

	// ensure WUF and RTC Alarm A flags are cleared
	__HAL_RTC_ALARM_CLEAR_FLAG(&hrtc, RTC_FLAG_ALRAF);
	__HAL_PWR_CLEAR_FLAG(PWR_FLAG_WU);
...
void RTC_AlarmConfig(void) {
	...
	// @ 23:15:30 every day generate Alarm event
	configAlarmA.AlarmTime.Hours = 23;
	configAlarmA.AlarmTime.Minutes = 15;
	configAlarmA.AlarmTime.Seconds = 30;
	...
	printMsg("Alarm set for 23:15:30 every day\r\n");
	}
