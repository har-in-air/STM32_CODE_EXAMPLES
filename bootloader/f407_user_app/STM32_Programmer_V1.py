...
def calc_file_len(binaryFileName):
    size = os.path.getsize(binaryFileName)
    return size

def open_the_file(binaryFileName):
    global bin_file
    bin_file = open(binaryFileName,'rb')
    #read = bin_file.read()
...
def decode_menu_command_code(command):
    ...
    if(command  == 0 ):
        ...
        fileName = input("Enter the application binary file name (E.g. user_app.bin):")

        #First get the total number of bytes in the .bin file.
        t_len_of_file = calc_file_len(fileName)
...
name = input("Enter the Port Name of your device(E.g. /dev/ttyUSB0):")
ret = 0
