...
// added HN
HAL_StatusTypeDef W25qxx_SpiTxBuffer(uint8_t* pBuffer, uint16_t numBytes){
	return HAL_SPI_Transmit(&_W25QXX_SPI, pBuffer, numBytes, HAL_MAX_DELAY);
	}
...
// added HN
// write upto 256 bytes in a single page using DMA
// Write cannot cross page boundary as that would require another page address set up, too much overhead
// for DMA write
// sw should check Lock == 0 before calling this function
// Note : write address = pageAddress*pageSize + pageOffset
void W25qxx_WritePageDMA(uint8_t *pBuffer, uint32_t pageAddress, uint32_t pageOffset, uint32_t numBytes){
	w25qxx.Lock = 1;
	if((pageOffset + numBytes) > w25qxx.PageSize){
		numBytes = w25qxx.PageSize - pageOffset;
		}
	// use normal HAL SPI r/w to set up the transfer
	W25qxx_WaitForWriteEnd();
	W25qxx_WriteEnable();
	CHIP_SELECT();
	uint32_t addr = pageAddress * w25qxx.PageSize + pageOffset;
	if (w25qxx.ID >= W25Q256){
		uint8_t cmd[] = {0x02, (uint8_t)(addr >> 24)&0xFF, (uint8_t)(addr >> 16)&0xFF, (uint8_t)(addr >> 8)&0xFF, (uint8_t)(addr &0xFF)};
		W25qxx_SpiTxBuffer(cmd, 5);
		}
	else {
		uint8_t cmd[] = {0x02, (uint8_t)(addr >> 16)&0xFF, (uint8_t)(addr >> 8)&0xFF, (uint8_t)(addr &0xFF)};
		W25qxx_SpiTxBuffer(cmd, 4);
		}
	// now link and enable the DMA for the data buffer transfer
	hal_spi_dma_init(&_W25QXX_SPI);
	HAL_SPI_Transmit_DMA(&_W25QXX_SPI, (uint8_t*)pBuffer, numBytes);
	// Chip deselect and DMA deinit (to allow normal spi r/w ) are done in the
...
// added HN
// de-link the DMA and deselect the chip select line
// main loop/task can check the lock flag to see if the DMA tx has completed

void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi){
	hal_spi_dma_deinit(&_W25QXX_SPI);
	CHIP_DESELECT();
	w25qxx.Lock = 0;
	}
...
// added HN
// write up to 256 bytes at any address (write can cross page boundary) using normal SPI writes
// sw should check for Lock == 0 before calling this function

void 	W25qxx_WriteBytes(uint8_t *pBuffer, uint32_t addr, uint32_t nBytes){
   ...
   if (nBytes <= nPageFreeBytes) {
	...
	w25qxx.Lock = 1;
	// wait if previous write has not completed
	W25qxx_WaitForWriteEnd();
	W25qxx_WriteEnable();
	CHIP_SELECT();
	if (w25qxx.ID >= W25Q256){
	...
	W25qxx_SpiTxBuffer(pBuffer, nP0Bytes);
	CHIP_DESELECT();
	...
	if (nP1Bytes) {
		...
		pBuffer += nP0Bytes;
		W25qxx_SpiTxBuffer(pBuffer, nP1Bytes);
		CHIP_DESELECT();
		}
	w25qxx.Lock = 0;
	}
...
// added HN
// read addres is the byte address
// no pageSize restriction on the number of bytes to read
// sw should check that Lock == 0 before calling this function
void W25qxx_ReadBytesDMA(uint8_t* pBuffer, uint32_t addr, uint32_t numBytes){
	w25qxx.Lock = 1;
	CHIP_SELECT();
	if (w25qxx.ID >= W25Q256){
		uint8_t cmd[] = {0x0B, (uint8_t)(addr >> 24)&0xFF, (uint8_t)(addr >> 16)&0xFF, (uint8_t)(addr >> 8)&0xFF, (uint8_t)(addr &0xFF), 0x00};
		W25qxx_SpiTxBuffer(cmd, 6);
		}
	else {
		uint8_t cmd[] = {0x0B, (uint8_t)(addr >> 16)&0xFF, (uint8_t)(addr >> 8)&0xFF, (uint8_t)(addr &0xFF), 0x00};
		W25qxx_SpiTxBuffer(cmd, 5);
		}

	hal_spi_dma_init(&_W25QXX_SPI);
	HAL_SPI_Receive_DMA(&_W25QXX_SPI, pBuffer, numBytes);

	// DMA de-init to allow regular spi functions, and de-assert of CS  are done in the
	// rx complete callback.
	}


// added HN
// on receive complete, de-init the DMA so that normal SPI r/w transactions can be used
// and de-assert the CS line. SW can use the Lock bit to check for receive complete
